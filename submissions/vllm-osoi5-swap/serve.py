#!/usr/bin/env python
from __future__ import annotations

import os
import subprocess
import sys


def _log(m: str) -> None:
    print("[serve] " + str(m), file=sys.stderr, flush=True)


def main() -> None:
    # Sync the shared community quantized model (chiku-inu's osoi5-v0-baked: int4 W4A16
    # g128 body + untied-int4 lm_head) from its bucket to a local dir, then serve it.
    # Same mechanism firfir-cast used. If WEIGHTS_BUCKET unset, fall back to MODEL_ID.
    weights_bucket = os.environ.get("WEIGHTS_BUCKET")
    model_id = (
        os.environ.get("SERVE_MODEL_OVERRIDE")
        or os.environ.get("MODEL_ID")
        or "google/gemma-4-E4B-it"
    )
    if weights_bucket:
        local = "/tmp/osoi5-model"
        _log("syncing weights %s -> %s" % (weights_bucket, local))
        rc = 1
        for cli in (["hf", "buckets", "sync"], ["huggingface-cli", "buckets", "sync"]):
            try:
                rc = subprocess.run(cli + [weights_bucket, local]).returncode
            except FileNotFoundError:
                continue
            if rc == 0:
                break
        if rc != 0:
            _log("weight sync FAILED rc=%s — aborting" % rc)
            sys.exit(1)
        model_id = local
        _log("model set to %s" % model_id)

    served_model_name = os.environ.get("SERVED_MODEL_NAME", "gemma-4-e4b-it")
    host = os.environ.get("HOST", "0.0.0.0")
    port = os.environ.get("PORT", "8000")
    max_model_len = os.environ.get("MAX_MODEL_LEN", "4096")
    gpu_memory_utilization = os.environ.get("GPU_MEMORY_UTILIZATION", "0.90")
    max_num_batched_tokens = os.environ.get("MAX_NUM_BATCHED_TOKENS")
    max_num_seqs = os.environ.get("MAX_NUM_SEQS")
    attention_backend = os.environ.get("ATTENTION_BACKEND")
    enable_prefix_caching = os.environ.get("ENABLE_PREFIX_CACHING")
    speculative_config = os.environ.get("SPECULATIVE_CONFIG")

    args = [
        sys.executable, "-m", "vllm.entrypoints.openai.api_server",
        "--model", model_id,
        "--served-model-name", served_model_name,
        "--host", host, "--port", port,
        "--dtype", "bfloat16",
        "--max-model-len", max_model_len,
        "--gpu-memory-utilization", gpu_memory_utilization,
        "--trust-remote-code",
        "--no-enable-log-requests",
    ]
    if max_num_batched_tokens:
        args += ["--max-num-batched-tokens", max_num_batched_tokens]
    if max_num_seqs:
        args += ["--max-num-seqs", max_num_seqs]
    if attention_backend:
        args += ["--attention-backend", attention_backend]
    if enable_prefix_caching:
        args += ["--enable-prefix-caching"]
    if speculative_config:
        args += ["--speculative-config", speculative_config]
    _log("launching vllm: model=%s backend=%s" % (model_id, attention_backend))
    os.execvpe(args[0], args, os.environ)


if __name__ == "__main__":
    main()
