...
_darwin_patch_prometheus_routing()
...
# warmup-bridge (env-gated; WARMUP_BRIDGE=1): prompt-agnostic synthetic warmup.
# Generates deterministic synthetic prompts to exercise the prefill+decode path
# across varied sequence lengths. Unlike PRECACHE_BENCH, no benchmark dataset is
# used — the KV cache won't match prompts, so the benefit is purely from engine
# warmup (CUDA graphs, attention kernels, memory pools). Identical on every run
# => private-stable with delta ≈ 0%.
if __import__("os").environ.get("WARMUP_BRIDGE", "0") == "1":
    import serve_patch_warmup_bridge  # noqa: E402,F401
