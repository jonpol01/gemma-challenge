# sitecustomize.py for mikasa-inbound/vllm-osoi5-pck04-v1
# ------------------------------------------------------------------
# Auto-imported by every interpreter on PYTHONPATH (serve.py puts this dir there),
# so this loads in vLLM's EngineCore subprocess where the model is built.
# Body is firfir-cast's shared `serve_patch_pck04.py` (from the gemma-challenge
# bucket) used VERBATIM — the proven pck04 lm_head fix. Replaces our v1
# zero-pad weight_loader hack, which mislocated the kept logits (ppl 4.3M).
# Activated by env PCK04_KEEPSET=/tmp/osoi5-model/pck04_keepset.json (K=16384).
# Verified against vLLM 0.23.0: Gemma4ForCausalLM.__init__/compute_logits +
# ParallelLMHead(org_num_embeddings=) signatures all match. The MTP drafter is
# class Gemma4MTPModel (not Gemma4ForCausalLM) so it is left untouched.
# ------------------------------------------------------------------
